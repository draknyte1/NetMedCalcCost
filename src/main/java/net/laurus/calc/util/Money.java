 ...
 * 1/5 and so on. <code>Money</code> objects are immutable. Like
 * {@link BigDecimal}, many operations return new <code>Money</code> objects. In
 * addition, most operations involving more than one <code>Money</code> object
 * will throw a <code>MismatchedCurrencyException</code> if the currencies don't
 * match.
 *
 ...
 *
 * <P>
 * This class takes either an <code>int</code> or a {@link BigDecimal} for its
 * multiplication and division methods. It doesn't take <code>float</code> or
 * <code>double</code> for those methods, since those types don't interact well
 * with <code>BigDecimal</code>. Instead, the <code>BigDecimal</code> class must
 * be used when the factor or divisor is a non-integer.
 *
 * <P>
...
public final class Money implements Comparable<Money>, Serializable {
	...
	/**
	 * Thrown when a set of <code>Money</code> objects do not have matching
	 * currencies.
	 *
	 * <P>
	...
	static {
		init(Currency.getInstance(Locale.UK), RoundingMode.HALF_EVEN);
	}
	 ...
	 *                      <code>Money</code> object in terms of 'thousands of
	...
	/**
	 * Return <code>true</code> only if <code>that</code> <code>Money</code> has the
	 * same currency as this <code>Money</code>.
	 */
	...
	public Money plus(Money that) {
		...
		return new Money(this.amount.add(that.amount), this.currency, this.rounding);
	}
	...
	public Money minus(Money that) {
		checkCurrenciesMatch(that);
		return new Money(this.amount.subtract(that.amount), this.currency, this.rounding);
	}

	/**
	 * Sum a collection of <code>Money</code> objects. Currencies must match. You
	 * are encouraged to use database summary functions whenever possible, instead
	 * of this method.
	 *
	 * @param moneys          collection of <code>Money</code> objects, all of the
	 *                        same currency. If the collection is empty, then a zero
	 *                        value is returned.
	 * @param currencyIfEmpty is used only when <code>moneys</code> is empty; that
	 *                        way, this method can return a zero amount in the
	 *                        desired currency.
	 */
	 ...
	 *
	 * <P>
	 * Return <code>true</code> only if the amounts are equal. Currencies must
	 * match. This method is <em>not</em> synonymous with the <code>equals</code>
	 * method.
	 */
	 ...
	 *
	 * <P>
	 * Return <code>true</code> only if 'this' amount is greater than 'that' amount.
	 * Currencies must match.
	 */
	 ...
	 *
	 * <P>
	 * Return <code>true</code> only if 'this' amount is greater than or equal to
	 ...
	 *
	 * <P>
	 * Return <code>true</code> only if 'this' amount is less than 'that' amount.
	 * Currencies must match.
	 */
	 ...
	 *
	 * <P>
	 * Return <code>true</code> only if 'this' amount is less than or equal to
	 * 'that' amount. Currencies must match.
	 */
	...
	/**
	 * Multiply this <code>Money</code> by an non-integral factor (having a decimal
	 * point).
	 *
	 * <P>
	 * The scale of the returned <code>Money</code> is equal to the scale of 'this'
	...
	public Money times(double factor) {
		BigDecimal newAmount = this.amount.multiply(asBigDecimal(factor));
		newAmount = newAmount.setScale(getNumDecimalsForCurrency(), this.rounding);
		return new Money(newAmount, this.currency, this.rounding);
	}
	...
	public Money div(double divisor) {
		BigDecimal newAmount = this.amount.divide(asBigDecimal(divisor), this.rounding);
		return new Money(newAmount, this.currency, this.rounding);
	}
	...
	/**
	 * Like {@link BigDecimal#equals(java.lang.Object)}, this <code>equals</code>
	 * method is also sensitive to scale.
	 *
	 * For example, <code>10</code> is <em>not</em> equal to <code>10.00</code> The
	...
	public int compareTo(Money that) {
		...
		comparison = this.currency.getCurrencyCode().compareTo(that.currency.getCurrencyCode());
		if (comparison != EQUAL)
			return comparison;
	...
	private void validateState() {
		...
		if (this.amount.scale() > getNumDecimalsForCurrency()) {
	...
	private void checkCurrenciesMatch(Money aThat) {
		if (!this.currency.equals(aThat.getCurrency())) {
			throw new MismatchedCurrencyException(
					aThat.getCurrency() + " doesn't match the expected currency : " + this.currency);
		}
	}
